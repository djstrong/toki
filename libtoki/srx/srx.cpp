...
namespace Toki { namespace Srx {
	...
	void SrxDocument::load(std::istream &is)
	{
		xmlpp::DomParser dom;
	...
	void SrxDocument::process_header_node(const xmlpp::Node */*n*/)
	{
		//no action yet
	}

	void SrxDocument::process_languagerule_node(const xmlpp::Node *n)
	{
	...
	void SrxDocument::process_rule_node(const std::string &language, const xmlpp::Node *n)
	{
		const xmlpp::Element* el = dynamic_cast<const xmlpp::Element*>(n);
	...
	void SrxDocument::process_languagemap_node(const xmlpp::Node */*n*/)
	{
		//TODO
	}

	std::vector<Rule> SrxDocument::get_all_rules() const
	{
		std::vector<Rule> v;
	...
	std::vector<Rule> SrxDocument::get_rules_for_lang(const std::string &/*lang*/) const
	{
		// TODO make it real
		return get_all_rules();
	}

	std::string SrxDocument::info() const
	{
		std::stringstream ss;
